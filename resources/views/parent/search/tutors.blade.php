...
@section('content')
<div class="main my-0 tutors-search">
    <div class="row mx-3 align-items-start">
        ...
        <div class="tutors-container w-80 px-3">
            @each('parent.search.tutor-card', $tutors, 'tutor')
        </div>
...
@push('js')
<script>
    $('.tutors-search .filter').submit(function (e) {
        e.preventDefault();
        e.stopPropagation();

        $('.tutor-card').fadeOut('fast');
        ...
        let searchQuery = `.tutor-card${genderQuery}${cityQuery}`;


        let matched = $(searchQuery);
        $('.tutors-container').find('.empty-result-set').remove();

        if (ageMatchedElementsCount(matched, minAge, maxAge) == 0) {
            $('.tutors-container').append(`<div class="empty-result-set text-center text-danger p-5 bg-white my-3 bold">
                No results found
            </div>`)
        }
        matched.each((_, e) => {
